...
/**
 * 正确的 MockResultSet（修复所有坑）
 * 模拟返回：user.name / role.role_code / permission.url 格式的列
 */
public class MockTestResultSet implements ResultSet, ResultSetMetaData {
    // 真实的测试数据行
    private final List<Map<String, Object>> rows = new ArrayList<>();
    // 当前行索引
    private int currentRow = -1;
    // 列名列表（固定是带点的格式）
    private final List<String> columnLabels = Arrays.asList("user.name", "role.role_code", "permission.url");

    public MockTestResultSet() {
        // 初始化测试数据：4行
        addRow("张三", "admin", "/user/list");
        addRow("张三", "admin", "/user/edit");
        addRow("张三", "operate", "/order/list");
        addRow("李四", "test", "/test/case");
    }

    // 添加一行测试数据
    private void addRow(String userName, String roleCode, String permUrl) {
        Map<String, Object> row = new HashMap<>();
        row.put("systemuser.name", userName);
        row.put("role.role_code", roleCode);
        row.put("permission.url", permUrl);
        rows.add(row);
    }
    ...
    public Object getObject(String columnLabel) throws SQLException {
        ...
        return rows.get(currentRow).get(columnLabel);
    }
    ...
    // ==================== ResultSetMetaData 核心方法 ====================
    @Override
    public int getColumnCount() {
        return columnLabels.size(); // 3列
    }
    ...
    public String getColumnLabel(int column) throws SQLException {
        ...
        return columnLabels.get(column - 1); // 索引从1开始
    }
   ...
   @Override public String getString(int columnIndex) throws SQLException { return getString(getColumnLabel(columnIndex)); }
    @Override public Object getObject(int columnIndex) throws SQLException { return getObject(getColumnLabel(columnIndex)); }
